...
def system_health():
    ...
    # Revenue Estimation (Completed/Partial Batches Only)
    # Lifetime Revenue
    c.execute("""
...
def list_history():
    ...
    # Fetch Page Data with Price for Value Calculation
    c.execute(f"""
...
# --- GLOBAL JOB ACTIONS (Cancel / Retry / Refund) ---
...
def job_action():
    ...
    if act == 'cancel':
        ...
        if row and row[2] != 'REFUNDED':
            ...
            # 2. Get User Price to Calculate Amount
...
def user_action():
    ...
    if act == 'reset_pass':
        # Using the admin-provided password from prompt
        ...
        # Update/Insert specific pricing
        ...
        # If priority, update base default too
        if ltype == 'priority': 
